...
local log_level = globalCfg.mysql.log_level
local mysql_shared_data = ngx.shared.mysql_proxy_app_logs
local last_date = string.sub(ngx.localtime(), 1, 10):gsub("[-]", "")
local full_log_name = globalCfg.mysql.app_log_path .."/mysql_program_" ..last_date.. ".log"
...
local function add_log(level, fmt, ...)
    ...
    local success, err, forcible = mysql_shared_data:lpush("mysql_app_log_queue", applog)
    if not success then
...
local function process_mysql_app_logs(premature)
    if premature then
        return
    end
    ...
    if now_date ~= last_date then
        last_date = now_date
        full_log_name = globalCfg.mysql.app_log_path .."/mysql_program_" ..last_date.. ".log"
    end
    ...
    if justStart == true then
        justStart = false
        local applog = string.format("[%s][Info ][%s]: mysql_proxy begin running", ngx.localtime(), ngx.worker.pid())
        file:write(applog.."\n")
    end
    ...
    while processed_count < batch_size do
        local currentlog = mysql_shared_data:rpop("mysql_app_log_queue")
    ...
    local ok, err = ngx.timer.at(1, process_mysql_app_logs)
    if not ok then
...
function _M.init()
    local ok, err = ngx.timer.at(0, process_mysql_app_logs)
    if not ok then
        ngx.log(ngx.ERR, "Failed to create process_mysql_app_logs timer: ", err)
        return false, err
    end
    return true
end
